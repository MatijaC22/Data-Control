    ...
    <v-row>
      ...
      <!-- <v-col cols="12" sm="6" md="4">
        <v-text-field
          ...
          hide-details
        ></v-text-field>
        <span v-if="submitClicked && userId == ''" style="color:red;">{{userIdAlarm}}</span>
      </v-col> -->
      <v-col cols="12" sm="6">
        <v-text-field
          label="Job id*"
          ...
          hide-details
        ></v-text-field>
        <span v-if="submitClicked && isNaN(JobId) == ''" style="color:red;">{{JobIdAlarm}}</span>
      </v-col>
      <v-row>
        <v-col cols="12" sm="6" >
          <v-file-input
...
<script>
// import deleteImage from '@/functions/insertOrUpdate'
...
export default {
  ...
  data() {
    return{
      // Title:toRaw(this.uniqueItem).Title != undefined ? toRaw(this.uniqueItem).Title : '',
      Id: toRaw(this.uniqueItem).id != undefined ? toRaw(this.uniqueItem).id : null,
      Description:toRaw(this.uniqueItem).description != undefined ? toRaw(this.uniqueItem).description : '',
      UserId:toRaw(this.uniqueItem).user_id != undefined ? toRaw(this.uniqueItem).user_id : JSON.parse(localStorage.getItem('userData')).id,
      JobId:toRaw(this.uniqueItem).job_id != undefined ? toRaw(this.uniqueItem).job_id : 1,
      ...
      UserIdAlarm:'Enter post worker!',
      JobIdAlarm:'Enter post job!',
      submitClicked:false,
  ...
  methods:{
    ...mapActions(useCounterStore, ['logout','updatePost','insertPost','uploadImages']),
    
    submitButtonPushed(){
      this.submitClicked = true
    },
    async submitData(values){
      if(this.Description && this.JobId && this.UserId){
        ...
        let image_name_folder = this.id +''
        if(this.Id){
          this.updatePost(userData)
          // OVDJE FALI DA AKO UPDATE POST NIJE STATUS CODE 200 DA ONDA NE SPREMI SLIKE
          if(this.selectedImages.length){
            
            this.uploadImages(image_name_folder, this.selectedImages)
          }
        }else{
          this.insertPost(userData)
          // OVDJE FALI DA AKO INSERT POST NIJE STATUS CODE 200 DA ONDA NE SPREMI SLIKE
          if(this.selectedImages.length){
