...
from db.models import DbNotifications, DbUsers, DbComments
import datetime
...
def create(db: Session, request, current_user:DbUsers):
    # Extract all who comment this post
    results = (
      db.query(DbComments.email)
      .filter(DbComments.post_id == request.post_id)
      .distinct()
      .all()
    )
...
def get_all_from_user(db: Session, id: int, current_user:DbUsers):
    user_notifications = db.query(DbNotifications).filter(DbNotifications.user_id == id).\
      order_by(DbNotifications.created_at.desc()).all()
    ...
    #                         detail = f"Notification from user {id} not found!")
    
    # user_maker = db.query(DbUsers).filter(DbUsers.id == user_notifications[0].comment_owner_id).first()
    ...
    for notification in user_notifications:
      user_maker = db.query(DbUsers).filter(DbUsers.id == notification.id).first()
...
def update_notification(db:Session, id: int, current_user:DbUsers):
    notification = db.query(DbNotifications).filter(DbNotifications.id == id).first()
    # notification = (
    # db.query(DbNotifications, DbUsers.email)
    # .join(DbUsers, DbNotifications.comment_owner_id == DbUsers.id)
